...
use vhost_user_frontend::{
    Generic, VhostUserConfig, VirtioDevice, VirtioDeviceType as VhostUserDeviceType,
};
...
pub struct VhostUserVsock {
    ...
    pub vhost_user: Mutex<Generic>,
    pub socket_path: String,
}

impl VirtioDeviceT for VhostUserVsock {
    fn new(
        config: &DeviceConfig,
        ...
        let vhost_user = Generic::new(
            vu_cfg,
            SeccompAction::Allow,
            EventFd::new(EFD_NONBLOCK).unwrap(),
            VhostUserDeviceType::Vsock,
        )
        ...
        let vsock = Arc::new(Mutex::new(VhostUserVsock {
...
impl Borrow<VirtioConfig<Queue>> for VhostUserVsock {
    fn borrow(&self) -> &VirtioConfig<Queue> {
        &self.virtio.config
    }
}

impl BorrowMut<VirtioConfig<Queue>> for VhostUserVsock {
    fn borrow_mut(&mut self) -> &mut VirtioConfig<Queue> {
        &mut self.virtio.config
    }
}

impl VirtioDeviceType for VhostUserVsock {
...
impl VirtioDeviceActions for VhostUserVsock {
    type E = Error;
...
impl VirtioMmioDevice for VhostUserVsock {
...
impl MutDeviceMmio for VhostUserVsock {
